...
private:
    typename Node* insert_recursively(typename KeyType key, typename DataType data, typename Node* node/*, stack<Node*>& path*/, bool& is_ok); // рекурсивное включение данных по ключу
public:
    int getSize();
    bool isEmpty();
    bool remove(typename KeyType key);
private:
    typename Node* remove_recursively(typename KeyType key, typename Node* node, bool& is_ok); // рекурсивное удаление данных по ключу
...
public:
    ...
    iterator end() { // возвращает nullptr
        return nullptr;
    }
    ...
    reverse_iterator rend() { // возвращает nullptr
        return nullptr;
    }
